...
def _apply_hubbard_logic(config, input_data):
    """
    Helper function to apply Hubbard configuration logic.
...
def test_lda_plus_u_not_set_for_new_format():
    ...
    input_data = {'SYSTEM': {}}
    _apply_hubbard_logic(config, input_data)
    
    # Verify that lda_plus_u is NOT set for new format
    assert 'lda_plus_u' not in input_data['SYSTEM']
