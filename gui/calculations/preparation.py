...
class CalculationPreparation(BaseCalculationPreparation):
    ...
    def prepare(self) -> Tuple[Atoms, Espresso]:
        ...
        if enable_magnetism or enable_hubbard:
            ...
            if magnetic_config:
                logger.info(
                    f"Setting up magnetic/Hubbard configuration: {magnetic_config}"
                )
                ...
                pseudopotentials = mag_result[
                    "pseudopotentials"
                ]  # Updated with species
                input_ntyp = mag_result.get(
                    "input_ntyp", {}
                )  # starting_magnetization, Hubbard_U

                logger.info(
                    f"Magnetic/Hubbard setup complete. Species: {list(pseudopotentials.keys())}"
                )
            else:
        ...
        if "kspacing" in config:
            ...
            # config["kspacing"] is in Angstrom^-1, so we need to divide by 2*pi
            kpts = kspacing_to_grid(atoms, config["kspacing"] / (2 * np.pi))
        ...
        if "queue" in config and config["queue"]:
            ...
            calc_params["queue"] = {"use_modules": True, "modules": config["modules"]}
