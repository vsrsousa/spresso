...
class CalculationPreparation(BaseCalculationPreparation):
    ...
    def prepare(self) -> Tuple[Atoms, Espresso]:
        ...
        if 'kpts' in config:
            ...
            # Convert kspacing to kpts using ASE
            try:
                from ase.io.espresso import kspacing_to_grid
                calc_params['kpts'] = kspacing_to_grid(atoms, config['kspacing'])
            except ImportError:
                logger.warning("ASE not available for kspacing_to_grid, using default kpts")
