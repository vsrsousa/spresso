...
"""
    ...
    def _on_file_selected(self, item, column):
        ...
        if path and os.path.isfile(path):
            try:
                ...
                stat = os.stat(path)
                info = (
                    f"Size: {stat.st_size} bytes | Lines: {len(content.splitlines())}"
                )
    ...
    def _update_status_and_config(self, run_tab=False):
        ...
        if not config or (
            not config.get("pseudopotentials") and not config.get("calc_type")
        ):
            status_label.setText(
...
"""
    ...
    def _generate_input_preview(self, atoms, config, label):
        ...
        lines.append("")
        lines.append(
            "! Note: This is a preview. Full input requires atomic positions and cell."
        )
        lines.append(
